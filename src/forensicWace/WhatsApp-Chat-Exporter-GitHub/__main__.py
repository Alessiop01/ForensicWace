...
import sqlite3
import shutil
#import json        # MODIFICATION: Removed because json functionality has been removed
import glob
import extract_exported, extract_iphone
import extract, extract_iphone_media
from utility import check_update
...
def main():
    ...
    parser.add_argument(        # MODIFICATION: Added password argument to the parser
        "-psw",
        "--password",
        dest="password",
        default="password",
        help="Password to decrypt backup (default: password)")
    parser.add_argument(
        '-j',
        '--json',
        dest='json',
    ...
    """if args.android:
        ...
        if os.path.isfile(contact_db):
            with sqlite3.connect(contact_db) as db:
                ...
                contacts(db, data)
    elif args.iphone:"""    # MODIFICATION: Removed android functionality
    if args.iphone:     # MODIFICATION: Added if statement in change of "elif" statement
        import sys
        ...
        if args.backup is not None:
            if not os.path.isdir(args.media):
                extract_iphone_media.extract_media(args.backup, args.password)      # MODIFICATION: Added password field
            else:
    ...
    # MODIFICATION: Removed unused functionalities FORENSIC WACE TOOL
    """
